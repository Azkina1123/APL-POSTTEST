...
void resetDaftarLagu();

void notifResetData(int ya, int tidak);
void pintuKeluar();
...
void resetDataTxt();
void ambilDataTxt();
void tambahKeTxt(Song lagu);
void hapusDariTxt(Song lagu);
void ubahPadaTxt(Song lagu);
...
int main()  {
    ...
    while (running){
        ...
        cout << "\t      "; color(opsi0); cout << char(16); color(7); cout << " Keluar        "; // opsi 0
        cout << "\t"      ; color(opsi3); cout << char(16); color(7); cout << " Reset playlist\n\n"; // opsi 0
...
void hasilPencarian(string cari, string atribut, Song arrayDitemukan[]) {
    ...
    for (int i=0; i<banyakDataTersimpan(); i++) {
        if (isSpace(arrayDitemukan[i].title)) {
            length = i;
            break;
        }
    }
        ...
        switch(opsi) {
            ...
            case -32:
                ...
                if (nav == SELECT && key == LEFT) {
                    ...
                    // tambahkan lagu ke file external
                    tambahKeTxt(laguBaru);
                    openPage = false;
                    break;
        ...
        switch (opsi) {
            ...
            case ENTER:
                ...
                // benar hapus
                    ...
                    // hapus semua data pada struct
                    for (int i=0; i<STORAGE; i++){
                        
                        // pointer lagu
                        Song *laguPtr = &songs[i];
...
void notifResetData(int ya, int tidak) {
    ...
    gotoxy(11, 10); cout << "---------------------------------------";
    gotoxy(11, 12); cout << "|                                     |";
    gotoxy(11, 13); cout << "|   Apakah Anda yakin ingin mereset   |";
    gotoxy(11, 14); cout << "|     seluruh lagu pada playlist?     |";
    gotoxy(11, 15); cout << "|                                     |";
    gotoxy(11, 16); cout << "|                                     |";
...
int indexLagu(Song lagu){
    for (int i=0; i<panjangArray(songs); i++){
        if (
            songs[i].title  == lagu.title &&
            songs[i].artist == lagu.artist &&
            songs[i].album  == lagu.album &&
            songs[i].year   == lagu.year
        ){
            return i;
        }
    }
...
int banyakDataTersimpan() {
    file.open("2109106004_AZIIZAH OKI SHOFRINA_APL_Posttest7_Data.txt", ios::in);
...
void ambilDataTxt() {
    ...
    string temp;
    int index = 0,
        banyakData = banyakDataTersimpan();

    file.open("2109106004_AZIIZAH OKI SHOFRINA_APL_Posttest7_Data.txt", ios::in);

    // masukkan atribut lagu tersimpan ke array struct
    while(!file.eof()) {

        // jika file kosong || sampai ke index terakhir
        if (banyakData == 0 || index == banyakData) {
            break;
        }

        getline(file, lagu.title, '|');

        getline(file, lagu.artist, '|');
        getline(file, lagu.album, '|');
        getline(file, temp, '|');
        lagu.duration = stof(temp);

        getline(file, lagu.genre[0], ',');
        getline(file, lagu.genre[1], ',');
        getline(file, lagu.genre[2], '|');

        getline(file, temp, '\n');
        lagu.year = stoi(temp);

        // ubah data lagu di index
        *(&songs[index]) = lagu;
        index++;
...
// tambahkan lagu tertentu
void tambahKeTxt(Song lagu) {

    file.open("2109106004_AZIIZAH OKI SHOFRINA_APL_Posttest7_Data.txt", ios::app);

    // tuliskan atribut lagu ke txt
    file << lagu.title    << '|'
         << lagu.artist   << '|'
         << lagu.album    << '|'
         << lagu.duration << '|'
...
// reset semua data eksternal
void resetDataTxt() {
    // hapus file
    int hapus = remove("2109106004_AZIIZAH OKI SHOFRINA_APL_Posttest7_Data.txt");
}

// hapus lagu tertentu
void hapusDariTxt(Song lagu) {
    int banyakData = banyakDataTersimpan(),
        index      = indexLagu(lagu);

    cout << "index = " << index << endl;

    // buat baru file eksternal
    file.open("2109106004_AZIIZAH OKI SHOFRINA_APL_Posttest7_Data.txt", ios::out);
    file.close();

    // tambahkan atribut tiap lagu ke txt
    for (int i=0; i<banyakData; i++) {
        if (i == index) {
            continue;

        } else if (isSpace(songs[i].title)) {
            break;
        }
...
void ubahPadaTxt(Song lagu) {
    ...
    // write ulang isi file eksternal
    file.open("2109106004_AZIIZAH OKI SHOFRINA_APL_Posttest7_Data.txt", ios::out);

    // tuliskan atribut tiap lagu ke txt
    for (int i=0; i<panjangArray(songs); i++) {
        // tuliskan atribut lagu ke txt
        file << songs[i].title    << '|'
             << songs[i].artist   << '|'
